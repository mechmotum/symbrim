...
if TYPE_CHECKING:
    ...
    from sympy import Basic, Expr, Matrix
...
class TestRollingDisc:
    ...
    def _rolling_disc_brim(self) -> None:
        self.rolling_disc: RollingDisc = RollingDisc("rolling_disc")
        self.rolling_disc.disc = KnifeEdgeWheel("disc")
        self.rolling_disc.disc.tyre_model = NonHolonomicTyreModel("tyre_model")
        self.rolling_disc.ground = FlatGround("ground")
        self.rolling_disc.define_kinematics()
        self.rolling_disc.define_loads()
        self.system: System = to_system(self.rolling_disc)
        str_vals: dict[str, float] = self._arbitrary_values()
        inertia: Matrix = self.rolling_disc.disc.body.central_inertia.to_matrix(
            self.rolling_disc.disc.frame)
        self.val_dict: dict[Basic, float] = {
    ...
    def _rolling_disc_manual(self) -> None:
        self.system: System = rolling_disc_manual()
        str_vals: dict[str, float] = self._arbitrary_values()
        self.val_dict: dict[Basic, float] = {
    ...
    def _set_values(self) -> None:
        self.p: tuple[Symbol, ...] = tuple(
            pi for pi in self.val_dict if isinstance(pi, Symbol))
        self.p_vals: tuple[float, ...] = tuple(self.val_dict[pi] for pi in self.p)
        self.q0: tuple[float, ...] = tuple(self.val_dict[qi] for qi in self.system.q)
        self.u0: tuple[float, ...] = tuple(self.val_dict[ui] for ui in self.system.u)
    ...
    def _plot_rolling_disc_manual(self, _rolling_disc_manual) -> None:
        ...
        plotter: SymMePlotter = SymMePlotter(
            ax, self.system.frame, self.system.origin)
        disc: RigidBody = self.system.get_body("disc")
        disc_plt: PlotBody = plotter.add_body(disc)
    ...
    def _plot_rolling_disc_brim(self, _rolling_disc_brim) -> None:
        ...
        fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
        plotter: Plotter = Plotter(ax, self.rolling_disc)
        plotter.lambdify_system((self.system.q, self.system.u, self.p))
        plotter.evaluate_system(self.q0, self.u0, self.p_vals)
        plotter.plot()
        ax.invert_zaxis()
    ...
    def test_rolling_disc_nonholonomic(self, method, request) -> None:
        ...
        fnh: list[Expr] = self.system.nonholonomic_constraints.xreplace(
